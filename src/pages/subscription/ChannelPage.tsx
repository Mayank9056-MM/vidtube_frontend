...
const mockChannel = {
  ...
  description: "Bringing you the latest in technology, reviews, tutorials, and industry insights. Join our community of tech enthusiasts!",
  isVerified: true,
  isSubscribed: false,
  totalVideos: 342,
  totalViews: "156M",
...
const Icon = ({ name, className = "w-5 h-5" }) => {
  const icons = {
    ...
    checkCircle: <svg className={className} fill="currentColor" viewBox="0 0 24 24"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>,
  ...
  return (
    <div className={`min-h-screen ${theme === "dark" ? "bg-black" : "bg-white"} transition-colors duration-300`}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        <button className={`flex items-center gap-2 ${theme === "dark" ? "text-gray-400 hover:text-white" : "text-gray-600 hover:text-black"} transition-colors`}>
          <Icon name="arrowLeft" />
          <span className="text-sm font-medium">Back to channels</span>
        </button>

        <div className={`overflow-hidden rounded-2xl ${theme === "dark" ? "bg-gray-900" : "bg-gray-50"} shadow-xl`}>
          <div className="h-32 sm:h-40 md:h-48 lg:h-56 relative" style={{ background: channel.coverImage }}>
          ...
          <div className="px-4 sm:px-6 lg:px-8 pb-6">
            <div className="flex flex-col sm:flex-row items-start gap-4 -mt-12 sm:-mt-16">
              <div
                ...
                style={{ background: channel.coverImage, borderColor: theme === "dark" ? "#111827" : "#ffffff" }}
              >
                ...
                <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      <h1 className={`text-2xl sm:text-3xl lg:text-4xl font-bold ${theme === "dark" ? "text-white" : "text-black"}`}>
                        {channel.channelName}
                      </h1>
                      {channel.isVerified && <Icon name="checkCircle" className="w-5 h-5 sm:w-6 sm:h-6 text-blue-500 flex-shrink-0" />}
                    </div>
                    <div className={`flex items-center gap-2 mb-3 text-sm sm:text-base ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>
                      <Icon name="users" className="w-4 h-4" />
                      <span className="font-medium">{channel.subscribers} subscribers</span>
                    </div>
                    <p className={`text-sm sm:text-base max-w-2xl ${theme === "dark" ? "text-gray-300" : "text-gray-700"}`}>
                      {channel.description}
                    </p>
                  </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className={`rounded-2xl p-6 ${theme === "dark" ? "bg-gradient-to-br from-blue-900/30 to-cyan-900/30" : "bg-gradient-to-br from-blue-50 to-cyan-50"} backdrop-blur-xl border ${theme === "dark" ? "border-blue-800/30" : "border-blue-200/50"} hover:shadow-xl transition-all duration-300`}>
            <div className="flex items-center gap-3 mb-3">
              <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-blue-500 to-cyan-500 flex items-center justify-center shadow-lg shadow-blue-500/30">
                <Icon name="video" className="w-6 h-6 text-white" />
              </div>
            </div>
            <p className={`text-sm mb-1 ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>Total Videos</p>
            <p className="text-3xl font-bold bg-gradient-to-r from-blue-600 to-cyan-600 bg-clip-text text-transparent">{channel.totalVideos}</p>
          </div>

          <div className={`rounded-2xl p-6 ${theme === "dark" ? "bg-gradient-to-br from-purple-900/30 to-pink-900/30" : "bg-gradient-to-br from-purple-50 to-pink-50"} backdrop-blur-xl border ${theme === "dark" ? "border-purple-800/30" : "border-purple-200/50"} hover:shadow-xl transition-all duration-300`}>
            <div className="flex items-center gap-3 mb-3">
              <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center shadow-lg shadow-purple-500/30">
                <Icon name="eye" className="w-6 h-6 text-white" />
              </div>
            </div>
            <p className={`text-sm mb-1 ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>Total Views</p>
            <p className="text-3xl font-bold bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent">{channel.totalViews}</p>
          </div>

          <div className={`rounded-2xl p-6 ${theme === "dark" ? "bg-gradient-to-br from-rose-900/30 to-orange-900/30" : "bg-gradient-to-br from-rose-50 to-orange-50"} backdrop-blur-xl border ${theme === "dark" ? "border-rose-800/30" : "border-rose-200/50"} hover:shadow-xl transition-all duration-300`}>
            <div className="flex items-center gap-3 mb-3">
            ...
            <p className={`text-sm mb-1 ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>Total Likes</p>
            <p className="text-3xl font-bold bg-gradient-to-r from-rose-600 to-orange-600 bg-clip-text text-transparent">{channel.totalLikes}</p>
          </div>

          <div className={`rounded-2xl p-6 ${theme === "dark" ? "bg-gradient-to-br from-emerald-900/30 to-teal-900/30" : "bg-gradient-to-br from-emerald-50 to-teal-50"} backdrop-blur-xl border ${theme === "dark" ? "border-emerald-800/30" : "border-emerald-200/50"} hover:shadow-xl transition-all duration-300`}>
            <div className="flex items-center gap-3 mb-3">
            ...
            <p className={`text-sm mb-1 ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>Engagement</p>
            <p className="text-3xl font-bold bg-gradient-to-r from-emerald-600 to-teal-600 bg-clip-text text-transparent">{channel.engagement}</p>
          </div>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className={`rounded-2xl p-6 ${theme === "dark" ? "bg-gray-900" : "bg-gray-50"} border ${theme === "dark" ? "border-gray-800" : "border-gray-200"}`}>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center">
                <Icon name="calendar" className="w-5 h-5 text-white" />
              </div>
              <div>
                <p className={`text-sm ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>Joined YouTube</p>
                <p className={`font-semibold ${theme === "dark" ? "text-white" : "text-black"}`}>{channel.joinedDate}</p>
              </div>
            </div>
          </div>
          ...
          <div className={`rounded-2xl p-6 ${theme === "dark" ? "bg-gray-900" : "bg-gray-50"} border ${theme === "dark" ? "border-gray-800" : "border-gray-200"}`}>
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className={`text-sm ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>Upload Frequency</p>
                <p className={`font-semibold ${theme === "dark" ? "text-white" : "text-black"}`}>{channel.uploadFrequency}</p>
              </div>
            </div>
          </div>
        </div>

        <div className={`rounded-2xl ${theme === "dark" ? "bg-gray-900" : "bg-gray-50"} border ${theme === "dark" ? "border-gray-800" : "border-gray-200"} overflow-hidden`}>
          <div className={`p-6 border-b ${theme === "dark" ? "border-gray-800" : "border-gray-200"}`}>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <h2 className={`text-xl font-bold ${theme === "dark" ? "text-white" : "text-black"}`}>All Videos</h2>
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {videos.map((video) => (
                <div
                  key={video.id}
                  className={`group rounded-xl overflow-hidden ${theme === "dark" ? "bg-gray-800/50" : "bg-white"} border ${theme === "dark" ? "border-gray-700/50" : "border-gray-200"} hover:shadow-xl transition-all duration-300 hover:scale-[1.02] cursor-pointer`}
                >
                  ...
                  <div className="p-4">
                    <h3 className={`font-semibold mb-2 line-clamp-2 ${theme === "dark" ? "text-white group-hover:text-red-400" : "text-black group-hover:text-red-600"} transition-colors`}>
                      {video.title}
                    </h3>
                    <div className={`flex items-center gap-3 text-xs ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>
