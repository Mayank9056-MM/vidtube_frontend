...
import { Clock, ThumbsUp, TrendingUp, Video } from "lucide-react";
...
export const Sidebar: React.FC<SidebarProps> = ({ theme, isOpen, toggleSidebar, isMobile }) => {
  const menuItems = [
  ...
  return (
    <>
      ...
      <aside
        className={`${
          isMobile ? 'fixed left-0 top-0 h-full z-50' : 'sticky top-16 h-[calc(100vh-4rem)]'
        } w-64 ${theme === 'dark' ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-200'} border-r transition-transform duration-300`}
      >
